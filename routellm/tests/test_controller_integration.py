...
def complex_controller(tmp_path):
    ...
    return Controller(
        ...
        traffic_manager=TrafficManager(
            rules=[
                TrafficRule(
                    pattern="special", strong_model="special-strong", weak_model="special-weak"
                )
            ]
        ),
...
async def test_controller_full_flow(complex_controller, monkeypatch):
    ...
    mock_res.model_dump.return_value = {
        "choices": [{"message": {"content": "integration success"}}]
    }
    ...
    res = await complex_controller.acompletion(
        router="random", threshold=0.5, messages=[{"role": "user", "content": "this is special"}]
    )
