...
class TestIntentModelSelector(unittest.TestCase):
    def setUp(self):
        ...
        self.intent_mappings = [
            IntentModelMapping(
                intent="marketing", model_pair=self.marketing_pair, description="Marketing content"
            ),
            IntentModelMapping(
                intent="technical", model_pair=self.technical_pair, description="Technical content"
            ),
        ]

        # Create the selector
        self.selector = IntentModelSelector(
            intent_mappings=self.intent_mappings, default_model_pair=self.default_pair
        )
...
class TestTierOnlySelectorSerialization(unittest.TestCase):
    ...
    def test_save_load_mappings_round_trips_a_pair_less_selector(self):
        ...
        try:
            ...
            self.assertEqual([m.intent for m in loaded.intent_mappings], ["legal", "code"])
    ...
    def test_intent_config_round_trips_a_pair_less_selector(self):
        ...
        try:
            ...
            self.assertEqual(sorted(m.intent for m in loaded.intent_mappings), ["code", "legal"])
            self.assertIsNone(loaded.default_model_pair)
            for mapping in loaded.intent_mappings:
                self.assertIsNone(mapping.model_pair)
            self.assertEqual(loaded.intent_tiers, self.selector.intent_tiers)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            f"Available files in middleware directory: {os.listdir(os.path.join(repo_root, 'routellm', 'middleware'))}"
        )
