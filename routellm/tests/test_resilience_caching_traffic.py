...
async def test_resilience_retry(controller, monkeypatch):
    ...
    success_res.model_dump.return_value = {
        "id": "test",
        "choices": [{"message": {"content": "success"}}],
    }

    mock_acompletion.side_effect = [MockError("Rate limit", 429), success_res]
...
async def test_resilience_fallback(controller, monkeypatch):
    ...
    fallback_res.model_dump.return_value = {
        "id": "test",
...
def test_traffic_manager_conditional_routing():
    rules = [TrafficRule(pattern="code", strong_model="codellama", weak_model="gpt-3.5-turbo")]
...
def test_load_balancer():
    config = LoadBalancerConfig(
        ...
        endpoints=[LoadBalancerEndpoint(model="m1"), LoadBalancerEndpoint(model="m2")],
    )
