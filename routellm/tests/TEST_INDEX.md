...
| `test_config_cli.py` | The `python -m routellm.config` inspection surface: `path` picks the winner, `paths` marks every searched location used or absent, `show` carries origin comments, and `--format json` for both | test_path_prints_the_winner, test_paths_marks_used_and_absent, test_show_carries_origin_comments (+ 6 more) |
