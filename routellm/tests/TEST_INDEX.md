...
| `test_capabilities.py` | Typed capabilities per endpoint (explicit block, deprecated tag aliases, models.dev record, and their precedence) | test_explicit_block_beats_the_catalog_record, test_catalog_record_fills_what_the_config_omits, test_unknown_stays_none_when_neither_source_knows (+ 9 more) |
| `test_capability_selectors.py` | Capability terms in a startup selector (`vision:`, `tools:`/`tool_call:`, `context:>=N`, `input:image`), the `max_output_desc` order, and when a failed catalog fetch is tolerable | test_vision_true_selects_only_vision_endpoints, test_tools_is_an_alias_of_tool_call, test_context_ge_filters_on_the_merged_context (+ 12 more) |
| `test_requirements.py` | Per-request requirements read off the OpenAI request (image parts, tools/functions, response_format, token count plus max_tokens and its fallback) | test_plain_text_request_has_no_requirements, test_image_url_part_sets_vision, test_tools_key_sets_tools (+ 15 more) |
| `test_capability_routing.py` | Capability-aware tier walk: exclude, force, or refuse before a level's router runs, the tier union, flat pairs, the fallback filter, strict, and the 400 refusal body | test_a_text_request_routes_exactly_as_before, test_a_vision_request_is_forced_to_the_only_capable_side, test_no_capable_side_raises_routing_error_naming_the_tier (+ 15 more) |
