...
def load_or_create_config(config_path: str | None = None) -> IntentModelSelector:
    ...
    marketing_model_pair = ModelPair(strong="gpt-4-turbo", weak="gpt-3.5-turbo")

    copywriting_model_pair = ModelPair(strong="claude-3-opus", weak="claude-3-sonnet")

    technical_model_pair = ModelPair(
        strong="gpt-4-turbo", weak="mistralai/Mixtral-8x7B-Instruct-v0.1"
    )
...
def create_web_ui(intent_selector: IntentModelSelector, config_path: str | None = None):
    ...
    domain_detector = DomainIntentDetector(
        ...
        cache_path=os.path.join(
            os.path.dirname(config_path) if config_path else ".", "intent_embeddings_cache.json"
        ),
    )
    ...
    def analyze_prompt(prompt: str):
        ...
        try:
            ...
            llm_scores_str = "\n".join(
                [f"{intent}: {score:.2f}" for intent, score in llm_scores.items()]
            )

            embedding_scores_str = "\n".join(
                [f"{intent}: {score:.2f}" for intent, score in embedding_confidence.items()]
            )
    ...
    def export_examples():
        ...
        try:
            export_path = os.path.join(
                os.path.dirname(config_path) if config_path else ".", "intent_examples.json"
            )
    ...
    with gr.Blocks(title="Intent-Based Routing") as demo:
        ...
        with gr.Tab("Analyze Prompt"):
            with gr.Row():
                ...
                with gr.Column():
                    ...
                    embedding_confidence_output = gr.Textbox(
                        label="Embedding Confidence Scores", lines=5
                    )
        ...
        with gr.Tab("Add Examples"):
            with gr.Row():
                with gr.Column():
                    example_prompt = gr.Textbox(
                        label="Example Prompt", placeholder="Enter an example prompt", lines=5
                    )
                    example_intent = gr.Dropdown(
                        label="Intent",
                        choices=[mapping.intent for mapping in intent_selector.intent_mappings],
                        value=intent_selector.intent_mappings[0].intent
                        if intent_selector.intent_mappings
                        else None,
                    )
            ...
            add_example_button.click(
                add_example, inputs=[example_prompt, example_intent], outputs=[add_result]
            )

            export_button.click(export_examples, inputs=[], outputs=[export_result])
...
def main():
    ...
    if args.create_config:
        config_path = (
            args.create_config if isinstance(args.create_config, str) else "intent_config.yaml"
        )
        # Ensure the directory exists
        os.makedirs(
            os.path.dirname(config_path) if os.path.dirname(config_path) else ".", exist_ok=True
        )
