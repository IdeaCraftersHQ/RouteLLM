...
technical_model_pair = ModelPair(strong="gpt-4-turbo", weak="mistralai/Mixtral-8x7B-Instruct-v0.1")
...
def visualize_confidence_scores(scores, best_match):
    ...
    for bar in bars:
        width = bar.get_width()
        plt.text(
            width + 1, bar.get_y() + bar.get_height() / 2, f"{width:.1f}", ha="left", va="center"
        )
...
def main():
    ...
    parser.add_argument(
        "--test", action="store_true", help="Run the test suite with example prompts"
    )
    parser.add_argument("--analyze", type=str, help="Analyze the intent of a specific prompt")
    parser.add_argument(
        "--visualize", action="store_true", help="Visualize confidence scores (requires matplotlib)"
    )
    ...
    parser.add_argument(
        "--router", type=str, default="random", help="Router to use (default: random)"
    )
    parser.add_argument(
        "--threshold", type=float, default=0.7, help="Routing threshold (default: 0.7)"
    )
    ...
    if args.test or args.analyze:
        try:
            ...
            if torch_available or args.router == "random":
                ...
                print(
                    f"Router '{args.router}' may require PyTorch. Falling back to 'random' router."
                )
                controller = Controller(
                    routers=["random"],
                    strong_model=default_model_pair.strong,
...
def run_test_suite(
    ...
    try:
        ...
        analyze_prompt_confidence(
            "I need help with my project that involves both design and coding", visualize
        )
    except Exception as e:
