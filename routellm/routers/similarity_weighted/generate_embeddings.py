...
def get_embeddings(battles_df):
    ...
    battles_df["first_turn"] = battles_df["prompt"].apply(lambda s: json.loads(s)[0].strip())
    ...
    for i in tqdm.tqdm(range(0, len(user_prompts), batch_size)):
        ...
        responses = client.embeddings.create(input=battles, model="text-embedding-3-small").data
